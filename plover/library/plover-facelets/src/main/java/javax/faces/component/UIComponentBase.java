 ...
 * more.
 *
 * @author Manfred Geiler (latest modification by $Author: martinkoci $)
 * @version $Revision: 1130268 $ $Date: 2011-06-01 14:14:09 -0500 (Wed, 01 Jun 2011) $
 *
    ...
    public void broadcast(FacesEvent event) throws AbortProcessingException
    {
        ...
        try
        {
            ...
            for (Iterator<FacesListener> it = _facesListeners.iterator(); it.hasNext();)
            {
                ...
                {
                    boolean failed = getFacesContext().isValidationFailed();
                    event.processListener(facesListener);
                    if (!failed && getFacesContext().isValidationFailed())
                        break;
                }
            }
        }
