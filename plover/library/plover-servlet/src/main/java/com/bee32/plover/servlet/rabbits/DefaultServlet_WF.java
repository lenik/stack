 ...
 * @author Lenik
 */
public class DefaultServlet_WF
        // Welcome-Fix.
        extends HttpServlet
    ...
    public void init()
        ...
        if (rrb != null) {
            try {
                ...
                if (root == null)
                    throw new UnavailableException("No base resourceBase for relativeResourceBase in"
            ...
            if (resource == null || !resource.exists())
                ...
                if (endsWithSlash && _aliases && pathInContext.length() > 1) {
                    ...
                    if (q != null && q.length() != 0)
                        pathInContext += "?" + q;
                    response.sendRedirect(response.encodeRedirectURL(URIUtil.addPaths(_context.getContextPath(),
                            pathInContext)));
                } else {
                ...
                if (!endsWithSlash
                        || (pathInContext.length() == 1 && request.getAttribute("org.mortbay.jetty.nullPathInfo") != null)) {
                    ...
                    if (_redirectWelcome) {
                        ...
                        if (q != null && q.length() != 0)
                            response.sendRedirect(response.encodeRedirectURL(URIUtil.addPaths(
                                    _context.getContextPath(), welcome) + "?" + q));
                        else
                            response.sendRedirect(response.encodeRedirectURL(URIUtil.addPaths(
                                    _context.getContextPath(), welcome)));
                    } else {
     ...
     *
     * @param pathInContext
     *            The path in Context in which we are looking for a welcome
    ...
    private String getWelcomeFile(String pathInContext)
        ...
        for (int i = 0; i < _welcomes.length; i++) {
            ...
            if (_welcomeServlets) {
                Map.Entry<Object, ServletHolder> entry = _servletHandler.getHolderEntry(welcome_in_context);
    ...
    protected void writeHeaders(HttpServletResponse response, HttpContent content, long count)
        ...
        if (response instanceof Response) {
            ...
            if (content.getLastModified() != null)
                fields.put(HttpHeaders.LAST_MODIFIED_BUFFER, content.getLastModified(), content.getResource()
                        .lastModified());
            ...
            if (_useFileMappedBuffer && resource.getFile() != null) {
                ...
                try {
                    ...
                    buffer = ((NIOConnector) connector).getUseDirectBuffers() ? (NIOBuffer) new DirectNIOBuffer(
                            (int) length) : (NIOBuffer) new IndirectNIOBuffer((int) length);
