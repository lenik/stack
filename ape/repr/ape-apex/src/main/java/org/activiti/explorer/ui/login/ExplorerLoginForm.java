...
public class ExplorerLoginForm extends LoginForm {
  ...
  protected byte[] getLoginHTML() {
    // Application URI needed for submitting form
      ...
      // http://localhost:39773/app2762/3/30/3/3/
    @SuppressWarnings("unused")
    String appUri = getApplication().getURL().toString();

    // 1
    String window = getWindow().getName();

    // XXX appUri is absoluted, however, localhost won't work in practice.
    HttpServletRequest request = ThreadServletContext.getRequest();

    // /app2762/3/30/3/3
    String apexPrefix = ILocationConstants.WEB_APP.join("3/30/3/3").resolveAbsolute(request);

    String prefix = apexPrefix + "/" + window + "/";
